...
public class DnsResponse {
    ...
    private ArrayList<byte[]> query = new ArrayList<>();
    ...
    public ArrayList<byte[]> getQuery() {
        return query;
    }
    ...
    private int parseQuery()
    {
        ...
        while(true) {
            int len = (int)this.response[offset++];
            if (len == 0) {
                break;
            }
            byte [] sub = new byte[len];
            for (int i = 0 ; i < len; ++i) {
                sub[i] = this.response[offset++];
            }
            query.add(sub);
        }
        byte[] qType = {this.response[offset], this.response[offset + 1]};
        offset += 2; // type
        this.queryType = this.getQTYPEFromByteArray(qType);

        offset += 2; // skip protocol class (IN/IN6)
        return offset;
    }
